...
/**
 * @see https://github.com/malyshev/yii-gravatar
 *
 * YiiGravatar class.
 *
...
class Gravatar extends Widget
{
    ...
    public function run()
    {
        ...
        $imageTag = Html::img($this->getImageUrl(),$this->htmlOptions);
        return $imageTag;
    }
    ...
    public function setRating($value)
    {
        ...
        if (false === in_array($value, $this->_ratings)) {
            throw new Exception(\Yii::t('application', 'Invalid rating value "{value}". Please make sure it is among ({enum}).',
    ...
    public function setDefaultImage($value)
    {
        if (false === (strpos($value, '://')) && false === in_array($value, $this->_defaultImages)) {
            throw new Exception(Yii::t('application', 'Invalid default image value "{value}". Please make sure it is among ({enum}) or it is absolute URL to the image file.',
    ...
    public function setSize($value)
    {
        ...
        if ($value < 1 || $value > 512) {
            throw new Exception(Yii::t('application', 'Invalid Gravatar size value "{value}". Please make sure it is between {min} and {max} (inclusive).',
